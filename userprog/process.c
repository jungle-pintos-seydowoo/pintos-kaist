 ...
 * 새로운 스레드는 process_create_initd()가 반환되기 전에 스케줄링되거나 종료될
 * 수 있습니다. 이 함수는 "한 번만 호출"되어야 합니다.
 *
...
static bool duplicate_pte(uint64_t *pte, void *va, void *aux) {
  ...
  /* 3. NOTE: Allocate new PAL_USER page for the child and set result to
   * NEWPAGE. */
  newpage = palloc_get_page(PAL_USER | PAL_ZERO);
  if (newpage == NULL) return false;

  /* 4. NOTE: Duplicate parent's page to the new page and check whether parent's
...
int process_exec(void *f_name) /* NOTE: 강의의 start_process() */
{
  ...
  for (token = strtok_r(NULL, " ", &saveptr); token != NULL;
...
void process_exit(void) {
  ...
  for (int idx = 2; idx < FDT_MAX; idx++) file_close(process_get_file(idx));
 ...
 * 현재 프로세스의 페이지 테이블을 파괴하고 커널 전용 페이지 테이블로 전환하는
 * 작업을 수행합니다.
 */
static void process_cleanup(void) {
  struct thread *curr = thread_current();
...
struct file *process_get_file(int fd) {
  /* fd 범위 체크 */
  if (fd < 2 || fd >= FDT_MAX) return NULL;
...
static bool load(const char *file_name, struct intr_frame *if_) {
  ...
  /* NOTE: [2.5] 파일 open 시 file_deny_write() 호출 / thread 구조체에 실행 중인
   * 파일 추가 */
...
static bool lazy_load_segment(struct page *page, void *aux) {
  ...
  // 1) 파일의 position을 ofs로 지정
  file_seek(lazy_load_arg->file, lazy_load_arg->ofs);
  // 2) 파일에서 read_bytes만큼 읽어서 upage에 적재
...
static bool setup_stack(struct intr_frame *if_) {
  bool success = false;
  void *stack_bottom = (void *)(((uint8_t *)USER_STACK) - PGSIZE); // 스택의 바닥주소 계산

  /* TODO: Map the stack on stack_bottom and claim the page immediately.
   * TODO: If success, set the rsp accordingly.
   * TODO: You should mark the page is stack. */
  /* TODO: Your code goes here */
  /* 1) stack_bottom 에 페이지를 하나 할당받음 */
  if(vm_alloc_page(VM_ANON | VM_MARKER_0, stack_bottom, 1))
  {
  /* 2) 할당 받은 페이지에 바로 물리 프레임을 매핑 */
    success = vm_claim_page(stack_bottom);
    if(success){
  /* 3) rsp 레지스터를 USER_STACK으로 설정 */
      if_->rsp = USER_STACK;  
    }
  }
