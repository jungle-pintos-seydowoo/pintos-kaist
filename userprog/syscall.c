...
pid_t exec(const char *cmd_line) {
  ...
  if (cmd_line_cpy == NULL) exit(-1);
  strlcpy(cmd_line_cpy, cmd_line, PGSIZE);

  if (process_exec(cmd_line_cpy) == -1) exit(-1);
}
...
int open(const char *file_name) {
  ...
  /* 파일 디스크립터 리턴*/
  if (file != NULL) fd = process_add_file(file);
...
int filesize(int fd) {
  ...
  int size = -1;
  /* 해당 파일의 길이를 리턴 */
  if (file != NULL) size = file_length(file);
...
int read(int fd, void *buffer, unsigned size) {
  ...
  /* 파일 디스크립터를 이용하여 파일 객체 검색 */
  struct file *file = process_get_file(fd);
  /* 파일 디스크립터가 0일 경우 키보드에 입력을 버퍼에 저장 후 버퍼의 저장한
   * 크기를 리턴 (input_getc() 이용) */
  if (fd == 0) {
  ...
  /* 파일 디스크립터가 0이 아닐 경우 파일의 데이터를 크기만큼 저장 후 읽은
   * 바이트 수를 리턴 */
  if (fd >= 2 && file) {
...
int write(int fd, const void *buffer, unsigned size) {
  ...
  /* 파일 디스크립터를 이용하여 파일 객체 검색 */
  struct file *file = process_get_file(fd);
  /* 파일 디스크립터가 1일 경우 버퍼에 저장된 값을 화면에 출력 후 버퍼의 크기
   * 리턴 (putbuf() 이용) */
  if (fd == 1) {
    putbuf(buffer, size);
    lock_release(&filesys_lock);
  ...
  /* 파일 디스크립터가 1이 아닐 경우 버퍼에 저장된 데이터를 크기만큼 파일에 기록
   * 후 기록한 바이트 수를 리턴 */
  if (fd >= 2 && file) {
...
unsigned tell(int fd) {
  ...
  unsigned position = -1;
  /* 해당 열린 파일의 위치를 반환 */
  if (file) position = file_tell(file);
  lock_release(&filesys_lock);
  return position;
}
...
/* ---------- UTIL ---------- */
/* NOTE: [2.2] 추가 함수 - 주소 값이 유저 영역에서 사용하는 주소 값인지 확인하는
 * 함수 */
void check_address(void *addr) {
  if (addr == NULL || is_kernel_vaddr(addr)) exit(-1);
}

/** @note [3.4] 메모리 매핑 파일에 필요한 함수들*/

/* @brief 인자의 유효성 확인 후 do_mmap() 호출 */
void *mmap(void *addr, size_t length, int writable, int fd, off_t offset) {
  // 주소가 page_align이 되었는지 여부
  if (!addr || addr != pg_round_down(addr)) {
    return NULL;
  }
  // offset이 page_align이 되었는 지 여부
